...
if ($action === 'update') {
  $fullName = trim((string)($data['fullName'] ?? '')); $employeeId = trim((string)($data['employeeId'] ?? '')); $email = trim((string)($data['email'] ?? '')); $department = trim((string)($data['department'] ?? '')); $role = trim((string)($data['role'] ?? ''));
  ...
  $check = $conn->prepare('SELECT userID FROM users WHERE (email = ? OR employeeId = ?) AND userID <> ? LIMIT 1'); $check->bind_param('ssi', $email, $employeeId, $userID); $check->execute();
