...
function Icon({ name }: { name: IconName }) {
  const paths: Record<IconName, JSX.Element> = {
    ...
    link: (
      <>
        <path d="M9 15 15 9" />
        <path d="M13 5.5 15 3.6a3.4 3.4 0 0 1 4.9 4.9L18 10.4M11 18.4l-1.9 1.9a3.4 3.4 0 0 1-4.9-4.9L6.1 13.5" />
      </>
    ),
  ...
  return (
    <div className="profile-menu-root" ref={rootRef}>
      ...
      >
        ...
        <span
          className={`profile-menu-chevron${open ? ' open' : ''}`}
        >
          <svg
            viewBox="0 0 24 24"
            width="13"
        ...
        <div className="profile-menu-dropdown" role="menu">
          ...
          <button
            ...
            onClick={() => {
              setOpen(false)
              navigate(
                profilePath.replace('/profile', '/settings'),
              )
            }}
          >
            <svg
...
function DeviceCard({
  ...
  return (
    <article
      className={`dashboard-card dm-card${
        expanded ? ' expanded' : ''
      }`}
    >
      <div
...
function AddDeviceModal({
  ...
  onClose: () => void
  onAdd: (device: DeviceDraft) => Promise<void>
  users: UserOption[]
}) {
  ...
  const validate = () => {
    ...
    if (!ip.trim()) {
      next.ip = 'IP address is required.'
...
function DeviceMonitoring({
  ...
  const currentUserName =
    ...
    getCurrentUserName() ||
    (isIT ? 'IT Personnel' : 'Administrator')
  ...
  useEffect(() => {
    ...
    /*
     * Read the latest profile photo immediately when
     * the component mounts.
     */
    updateProfilePhoto()

    /*
     * Listen for the custom event dispatched by
     * Admin/Profile.tsx after a successful upload.
     */
    ...
    >
      <aside className="admin-sidebar">
        ...
        <nav
          ...
          aria-label={`${currentUserRole} navigation`}
        >
      ...
      <main className="admin-main">
        <header className="admin-topbar">
          ...
          <div className="topbar-title">
            ...
            <p>
              Track the status and health of all
              network devices.
            </p>
          </div>
        ...
        <div className="dashboard-content">
          ...
          {loadingDevices ? (
            <div className="dm-grid">
              <div className="dashboard-card dm-empty">
                Loading devices…
              </div>
            </div>
          ) : deviceError ? (
            ...
            <div className="dm-grid">
              <div className="dashboard-card dm-empty">
                No devices match your search or
                filters.
              </div>
            </div>
          ) : (
